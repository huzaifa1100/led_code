...
// Constants:
// wifi details
// const char *ssid = F("WRLaccess");
// const char *password = F("12345678");
const char *ssid = F("WRLaccess");
const char *password = F("12345678");
...
const char *apiKey = F("c29928bf-4283-46d4-8a5c-a21b79eac40a");
// OTA Password
const char *OTAuserName = F("WhatsRunning");
const char *OTApassword = F("123qweE#");
// Other
const char *applicationJson = F("application/json");
const char *defaultColorCodes = F("255,0,0");
...
void initWiFI()
{
  ...
  int i = 0;
  while (WiFi.status() != WL_CONNECTED || (i > 10 && eth_connected))
  { // Wait for connection
    delay(500);
    i++;
...
void initLightAPIs()
{
  ...
  AsyncCallbackJsonWebHandler *turnOnColorsHandler = new AsyncCallbackJsonWebHandler("/turnOnColors", [](AsyncWebServerRequest *request, JsonVariant &json)
                                                                                     { authorizeWithBody(request, json, &turnOnColorsAPI); });
  server.addHandler(turnOnColorsHandler);

  // Turn off leds on the scanner
  // NoParameters
  server.on("/turnOffLeds", HTTP_POST, [](AsyncWebServerRequest *request)
            { authorize(request, &turnOffColorsAPI); });
  ...
  AsyncCallbackJsonWebHandler *flashHandler = new AsyncCallbackJsonWebHandler("/flash", [](AsyncWebServerRequest *request, JsonVariant &json)
                                                                              { authorizeWithBody(request, json, &flashLedsAPI); });
  server.addHandler(flashHandler);

  // reset to a default initial state, currently red
  // NoParameters
  server.on("/resetToDefault", HTTP_POST, [](AsyncWebServerRequest *request)
            { authorize(request, &resetToDefaultAPI); });
}
...
void initSoundAPIs()
{
  ...
  server.on("/playSound", HTTP_POST, [](AsyncWebServerRequest *request)
            { authorize(request, &playSoundAPI); });
  ...
  server.on("/soundVolume", HTTP_POST, [](AsyncWebServerRequest *request)
            { authorize(request, &changeVolumeAPI); });
}
...
void initFileAPIs()
{
  ...
  server.on("/getFilesInRootFolder", HTTP_GET, [](AsyncWebServerRequest *request)
            { authorize(request, &getFilesInRootFolderAPI); });

  // download file
  // Parameters:
  // filename - represents a string with the file name
  server.on("/downloadFile", HTTP_GET, [](AsyncWebServerRequest *request)
            { authorize(request, &downloadFile); });

  // delete file
  // Parameters:
  // filename - represents a string with the file name
  server.on("/deleteFile", HTTP_DELETE, [](AsyncWebServerRequest *request)
            { authorize(request, &deleteFile); });
...
String listFiles(bool ishtml)
{
  ...
  for (JsonObject file : files)
  {
    ...
    if (ishtml)
    {
      returnText += F("<tr align='left'><td>"+fileName+"</td><td>"+fileSize+"</td></tr>");
    }
    else
    {
      returnText += F("File: "+fileName+"\n");
    }
  }
...
{
  if (request->hasHeader(F("api-key")) == false or String(request->header(F("api-key")).c_str()) != apiKey)
  {
